...
Airplane::Airplane(string _model, string _airlines, size_t _year_of_manufacture, int _capacity, double _fullness, double _percent_avarage, int _weight, double _cargocapacity)
{
	model = _model;
	airlines = _airlines;
...
void Airplane::load(const int _kilograms)
{
	return;
}
...
void Airplane::printInfo() {
	cout << "  " << getModel() << " — " << getFullness() << " - " << getPercentAvarage() << "%" " - " << getWeight() << " т" << " - " << getCargocapacity() << "%" << endl;
}
